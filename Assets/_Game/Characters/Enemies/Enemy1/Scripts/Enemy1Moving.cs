...
public class Enemy1Moving : MonoBehaviour
{
    ...
    private float _angleÅrror = 0.1f;
    ...
    private void ChangeDirection(float leftPositon, float rightPosition)
    {
        ...
        {
            _directionMoving = Vector3.zero;
            _rigidbody.rotation = Quaternion.RotateTowards(_rigidbody.rotation, Quaternion.Euler(0, _angleÅrror, 0), _speedRotation);
            if (transform.eulerAngles.y < angleWhichEnemyChangesDirection)
            {
                _directionMoving = Vector3.right;
            }
        ...
        {
            _directionMoving = Vector3.zero;
            _rigidbody.rotation = Quaternion.RotateTowards(_rigidbody.rotation, Quaternion.Euler(0, 180 - _angleÅrror, 0), _speedRotation);
